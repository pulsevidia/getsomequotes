  ...
  return (
    <Card maw={600} style={{ cursor: 'pointer' }} padding="lg" radius="md" onClick={() => navigate(`/blog/${blog.$id}`)}>
      <Group style={{ flexWrap: "nowrap", alignItems: "flex-start" }}>
        ...
        <Stack gap={0}>
          <Badge mb={'xs'} variant="light" color={'violet'}>The Beginning of Infinity</Badge>
          <Title lineClamp={smallSizeMath ? 2 : 1} fw={600} style={{ fontFamily: "Afacad Flux" }} order={smallSizeMath ? 5 : 3}>{title}</Title>
          {!smallSizeMath &&
